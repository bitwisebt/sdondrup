        ...
        <section class="content">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Relation List</h3>

                    <div class="card-tools">
                        <button type="button" class="btn btn-tool" data-card-widget="collapse" title="Collapse">
                            <i class="fas fa-minus"></i>
                        </button>
                        <a class="btn btn-success btn-sm" href="{{route('relation.create')}}"><i class="fa fa-plus"></i> Add</a>
                    </div>
                </div>
                <div class="card-body">
                    <ul class="nav nav-links mb-3 mb-lg-2 mx-n3">
                        <li class="nav-item"><a class="nav-link active" href="/relation">All Relation <span class="text-700 fw-semi-bold">({{$relation_all}})</span></a></li>
                        <li class="nav-item"><a class="nav-link" href="/relation?archive">View Archived Relation <span class="text-700 fw-semi-bold">({{$relation_count}})</span></a></li>
                    </ul>
                            ...
                            <table class="table table-sm fs--1 mb-0" id="myTable" width="60%">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sort border-top" data-sort="name">Relation</th>
                                        <th class="sort text-center align-middle pe-0 border-top ps-1" scope="col">ACTION</th>
                                    </tr>
                                </thead>
                                <tbody class="list">
                                    @forelse($relations as $data)
                                    <tr>
                                        <td class="align-middle ps-3 id">{{$loop->iteration}}</td>
                                        <td class="align-middle name">{{$data->relation}}</td>
                                        
                                        <td class="align-middle white-space-nowrap text-center pe-0">
                                            @if($data->trashed())
                                            @can('Relation restore')
                                            <form method="POST" action="{{route('relation.restore',$data->id)}}">
                                                @csrf
                                                <button class="btn btn-sm btn-outline-secondary me-1 mb-1 fs--2" type="submit">Restore</button>
                                            </form>
                                            @endcan
                                            @can('Relation delete forever')
                                            <form action="{{route('relation.force_delete',$data->id)}}" method="POST">
                                                @csrf
                                            ...
                                            @else
                                            @can('Relation edit')
                                            <a class="btn btn-sm btn-phoenix-info me-1 fs--2 " href="{{route('relation.edit',$data)}}"><span class="fas fa-edit"></span></a>
                                            @endcan
                                            @can('Relation delete')
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <a class="btn btn-sm btn-danger text-white" href="{{route('relation.destroy',$data->id)}}"> Yes </a>
