            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        ...
                        <a class="btn btn-success btn-sm" href="{{route('course.create')}}"><i class="fa fa-plus"></i> Add</a>
                    </div>
                </div>
                <div class="card-body">
                    <ul class="nav nav-links mb-3 mb-lg-2 mx-n3">
                        <li class="nav-item"><a class="nav-link active" href="/Course">All Course <span class="text-700 fw-semi-bold">({{$course_all}})</span></a></li>
                        <li class="nav-item"><a class="nav-link" href="/Course?archive">View Archived Course <span class="text-700 fw-semi-bold">({{$course_count}})</span></a></li>
                    </ul>
                            ...
                            <table class="table table-sm fs--1 mb-0" id="myTable" width="60%">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sort border-top" data-sort="name">Course</th>
                                ...
                                <tbody class="list">
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle name">{{$data->course}}</td>
                                        
                                        <td class="align-middle white-space-nowrap text-center pe-0">
                                            @if($data->trashed())
                                            @can('course restore')
                                            <form method="POST" action="{{route('course.restore',$data->id)}}">
                                                @csrf
                                                <button class="btn btn-sm btn-outline-secondary me-1 mb-1 fs--2" type="submit">Restore</button>
                                            </form>
                                            @endcan
                                            @can('course delete forever')
                                            <form action="{{route('course.force_delete',$data->id)}}" method="POST">
                                                @csrf
                                            ...
                                            @else
                                            @can('course edit')
                                            <a class="btn btn-sm btn-phoenix-info me-1 fs--2 " href="{{route('course.edit',$data)}}"><span class="fas fa-edit"></span></a>
                                            @endcan
                                            @can('course delete')
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <a class="btn btn-sm btn-danger text-white" href="{{route('course.destroy',$data->id)}}"> Yes </a>
