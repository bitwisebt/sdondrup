            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        ...
                        <a class="btn btn-success btn-sm" href="{{route('university.create')}}"><i class="fa fa-plus"></i> Add</a>
                    </div>
                </div>
                <div class="card-body">
                    <ul class="nav nav-links mb-3 mb-lg-2 mx-n3">
                        <li class="nav-item"><a class="nav-link active" href="/university">All University <span class="text-700 fw-semi-bold">({{$university_all}})</span></a></li>
                        <li class="nav-item"><a class="nav-link" href="/university?archive">View Archived University <span class="text-700 fw-semi-bold">({{$university_count}})</span></a></li>
                    </ul>
                            ...
                            <table class="table table-sm fs--1 mb-0" id="myTable" width="60%">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sort border-top" data-sort="name">University</th>
                                        <th class="sort text-center align-middle pe-0 border-top ps-1" scope="col">ACTION</th>
                                    </tr>
                                </thead>
                                <tbody class="list">
                                    @can('University access')
                                    @forelse($university as $data)
                                    <tr>
                                        <td class="align-middle ps-3 id">{{$loop->iteration}}</td>
                                        <td class="align-middle name">{{$data->university}}</td>
                                        
                                        <td class="align-middle white-space-nowrap text-center pe-0">
                                            @if($data->trashed())
                                            @can('University restore')
                                            <form method="POST" action="{{route('university.restore',$data->id)}}">
                                                @csrf
                                                <button class="btn btn-sm btn-outline-secondary me-1 mb-1 fs--2" type="submit">Restore</button>
                                            </form>
                                            @endcan
                                            @can('University delete forever')
                                            <form action="{{route('university.force_delete',$data->id)}}" method="POST">
                                                @csrf
                                            ...
                                            @else
                                            @can('University edit')
                                            <a class="btn btn-sm btn-phoenix-info me-1 fs--2 " href="{{route('university.edit',$data)}}"><span class="fas fa-edit"></span></a>
                                            @endcan
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <a class="btn btn-sm btn-danger text-white" href="{{route('university.destroy',$data->id)}}"> Yes </a>
