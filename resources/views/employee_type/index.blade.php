            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        ...
                        <a class="btn btn-success btn-sm" href="{{route('employee_type.create')}}"><i class="fa fa-plus"></i> Add</a>
                    </div>
                </div>
                <div class="card-body">
                    <ul class="nav nav-links mb-3 mb-lg-2 mx-n3">
                        <li class="nav-item"><a class="nav-link active" href="/employee_type">All Type <span class="text-700 fw-semi-bold">({{$employee_all}})</span></a></li>
                        <li class="nav-item"><a class="nav-link" href="/employee_type?archive">View Archived Type <span class="text-700 fw-semi-bold">({{$employee_count}})</span></a></li>
                    </ul>
                            ...
                            <table class="table table-sm fs--1 mb-0" id="myTable" width="60%">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sort border-top" data-sort="name">Employee Type</th>
                                        <th class="sort text-center align-middle pe-0 border-top ps-1" scope="col">ACTION</th>
                                    </tr>
                                </thead>
                                <tbody class="list">
                                    @can('EmployeeType access')
                                    @forelse($employee as $data)
                                    <tr>
                                        <td class="align-middle ps-3 id">{{$loop->iteration}}</td>
                                        <td class="align-middle name">{{$data->type}}</td>
                                        
                                        <td class="align-middle white-space-nowrap text-center pe-0">
                                            @if($data->trashed())
                                            @can('EmployeeType restore')
                                            <form method="POST" action="{{route('employee_type.restore',$data->id)}}">
                                                @csrf
                                                <button class="btn btn-sm btn-outline-secondary me-1 mb-1 fs--2" type="submit">Restore</button>
                                            </form>
                                            @endcan
                                            @can('EmployeeType delete forever')
                                            <form action="{{route('employee_type.force_delete',$data->id)}}" method="POST">
                                                @csrf
                                            ...
                                            <a class="btn btn-sm btn-phoenix-info me-1 fs--2 " href="{{route('employee_type.edit',$data)}}"><span class="fas fa-edit"></span></a>
                                            @endcan
                                            @can('EmployeeType delete')
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <a class="btn btn-sm btn-danger text-white" href="{{route('employee_type.destroy',$data->id)}}"> Yes </a>
