                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">Proficiency Test</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
        ...
        <section class="content">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Proficiency Test List</h3>

                    <div class="card-tools">
                        <button type="button" class="btn btn-tool" data-card-widget="collapse" title="Collapse">
                            <i class="fas fa-minus"></i>
                        </button>
                        <a class="btn btn-success btn-sm" href="{{route('test.create')}}"><i class="fa fa-plus"></i> Add</a>
                    </div>
                </div>
                <div class="card-body">
                    <ul class="nav nav-links mb-3 mb-lg-2 mx-n3">
                        <li class="nav-item"><a class="nav-link active" href="/test">All Proficiency Test <span class="text-700 fw-semi-bold">({{$test_all}})</span></a></li>
                        <li class="nav-item"><a class="nav-link" href="/test?archive">View Archived Proficiency Test <span class="text-700 fw-semi-bold">({{$test_count}})</span></a></li>
                    </ul>
                            ...
                            <table class="table table-sm fs--1 mb-0" id="myTable" width="90%">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sort border-top" data-sort="name">Short Form</th>
                                ...
                                <tbody class="list">
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle name">{{$data->sht_name}}</td>
                                        <td class="align-middle name">{{$data->full_name}}</td>
                                        <td class="align-middle white-space-nowrap text-center pe-0">
                                            @if($data->trashed())
                                            @can('Test restore')
                                            <form method="POST" action="{{route('test.restore',$data->id)}}">
                                                @csrf
                                                <button class="btn btn-sm btn-outline-secondary me-1 mb-1 fs--2" type="submit">Restore</button>
                                            </form>
                                            @endcan
                                            @can('Test delete forever')
                                            <form action="{{route('test.force_delete',$data->id)}}" method="POST">
                                                @csrf
                                            ...
                                            @else
                                            @can('Test edit')
                                            <a class="btn btn-sm btn-phoenix-info me-1 fs--2 " href="{{route('test.edit',$data)}}"><span class="fas fa-edit"></span></a>
                                            @endcan
                                            @can('Test delete')
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <a class="btn btn-sm btn-danger text-white" href="{{route('test.destroy',$data->id)}}"> Yes </a>
