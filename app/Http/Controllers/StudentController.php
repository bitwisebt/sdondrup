...
class StudentController extends Controller
{
    ...
    public function create()
    {
        ...
        $student = Application::where('flag','N')->get();
        $agent = Agent::all();
        $visa=VisaType::all();
        return view('student.create', compact('qualification', 'enrol', 'test', 'relation','university','student','agent','visa'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $stid = 'SD-' . $request->registration_type . '-' . $tid;
        ...
        try {
           ...
           if($request->enrolment!=null){
            ...
            if ($request->registration_type == 'E') {
                $status = Status::whereIn('id', [1,2,3,4,5,8,9])
                    ->get();
            ...
            DB::table('applications')
                ...
                ->update(['flag' => 'Y']);
            Mail::raw($request->name . ' your registration with Sampai Dhondrup Education Consultancy has been successfully completed.Your application will be processed and should we require any additional document or further information, we will contact you.Thank you for trusting Sampai Dhondrup Education Consultancy!', function ($message) use ($request) {
                $message->from('system.sampaidhondrup@gmail.com', 'Sampia Dhondrup Education Consultancy');
    ...
    public function edit(string $id)
    {
        ...
        $students = Application::where('flag','N')->get();
        $visa=VisaType::all();
        $user=User::where('role','!=',9)->get();
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            if($std->registration_type==$request->registration_type){
            $std->update([
                'cid_number' => $request->cidno,
                'name' => $request->name,
                'gender' => $request->gender,
            ...
            Proficiency::where('student_id', $id)->forceDelete();
            Proficiency::create([
                'student_id' => $id,
                'test_id' => $request->test,
                'reading' => $request->reading,
            ...
            QualificationHistory::where('student_id', $id)->forceDelete();
            for ($i = 0; $i < count($request->qualification); $i++) {
                $qui = new QualificationHistory();
                $qui->student_id = $id;
                $qui->qualification_id = $request->qualification[$i];
            ...
            EmergencyContact::where('student_id', $id)->forceDelete();
            for ($i = 0; $i < count($request->emergency_name); $i++) {
                $emer = new EmergencyContact();
                $emer->student_id = $id;
                $emer->relation_id = $request->relation[$i];
            ...
            StatusHeader::where('student_id', $id)->forceDelete();
            if ($request->registration_type == 'E') {
                $status = Status::whereIn('id', [1,2,3,4,5,8,9])
                    ->get();
            } elseif ($request->registration_type == 'V') {
    ...
    public function destroy(string $id)
    {
        try {
            ...
            Application::where('name', $user->name)
              ->where('email', $user->email)
              ->update(['flag' => 'N']);
            $user->forceDelete();
            StudentTransaction::where('student_id',$id)->forceDelete();
            StatusHeader::where('student_id',$id)->forceDelete();
